...
c     if(abrt) ecorr=-99.               ! if no ellipticity correction available
      ...
      ! be used!
      if(del.ge.98) then
        write(*,*) 'Phase is changed to Pdiff'
        phase='Pdiff'
      else
        write(*,*) 'Phase is changed to P'
        phase='P'
      end if
...
c       write(13,*) 'ecorr,ecorr2=',ecorr,ecorr2
     ...
     &      rayq)
        ...
        !!!!!!!! Kasra
        p = 254.33023568789042
        !del = 160.0
        write(*,*) 'WARNINGNGNGNNGNGNGNGNGNGNGNGNGNG'
        write(*,*) 'ray parameter (FIXED) and distance (FIX): '
        write(*,*) p/180.*pi
        write(*,*) del
...
C 90        if(kd.ne.2.and.kd.ne.4) then
C             write(2,95) kseg,kd,ptlat,ptlon,rtarget,tau,telev
C 95          format(2i3,2f8.2,f8.1,2f8.3)
C           else
C             write(2,96) kseg,kd,0,0,rtarget,0,0
C 96          format(2i3,2i8,f8.1,2i8)
C           endif 

          if(jdebug.eq.1)
     &      write(13,*) 'Final crustal correction:',tau,' Elev:',telev
        write(*,*) 'Final crustal correction:',tau,' Elev:',telev
